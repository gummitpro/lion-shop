...
import { Table, Row, Col, Button, Input, Popconfirm, Badge, Space, Select } from 'antd';
    ...
    const columns = [
      ...
      },
      {
        ...
        <Space>
          ...
          onConfirm={() => changeUser({id: record.id ,status: !record.status})}
          okText="Chuyển"
          cancelText="Hủy"
        >
          <Button type='ghost' disabled={record.admin} icon={<DisconnectOutlined />}></Button>
        </Popconfirm>
        <Popconfirm
          ...
          cancelText="Hủy"
        >
          <Button danger disabled={record.admin} icon={<DeleteOutlined />}></Button>
        </Popconfirm>
      </Space>
  },
    ];
